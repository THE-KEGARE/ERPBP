...
script_version("1.2.3-2025-01-03")
...
local updater_loaded, Updater = pcall(loadstring, string.format([[
local encoding = require 'encoding'
encoding.default = 'CP1251'
return {check=function (a,b,c) 
    ...
    downloadUrlToFile(a,e,function(g,h,i,j)
        if h==d.STATUSEX_ENDDOWNLOAD then 
            if doesFileExist(e) then
                ...
                if k then
                    ...
                    if updateversion~=thisScript().version then
                        lua_thread.create(function(b)
                            local d=require('moonloader').download_status
                            local m=-1
                            local update_msg = encoding.UTF8:decode('Обнаружено обновление. Пытаюсь обновиться c '..thisScript().version..' на '..updateversion)
                            sampAddChatMessage(b..update_msg,m)
                            wait(250)
                            downloadUrlToFile(updatelink,thisScript().path,function(n,o,p,q)
                                if o==d.STATUS_DOWNLOADINGDATA then
                                    print(encoding.UTF8:decode(string.format('Загружено %%d из %%d.',p,q)))
                                elseif o==d.STATUS_ENDDOWNLOADDATA then
                                    print(encoding.UTF8:decode('Загрузка обновления завершена.'))
                                    sampAddChatMessage(b..encoding.UTF8:decode('Обновление завершено!'),m)
                                    goupdatestatus=true
                                    lua_thread.create(function()wait(500)thisScript():reload()end)
                                end
                                if o==d.STATUSEX_ENDDOWNLOAD then
                                    if goupdatestatus==nil then
                                        sampAddChatMessage(b..encoding.UTF8:decode('Обновление прошло неудачно. Запускаю устаревшую версию..'),m)
                                        update=false
                                    end
                                end
                            end)
                        ...
                        update=false
                        print(encoding.UTF8:decode('v'..thisScript().version..': Обновление не требуется.'))
                        if l.telemetry then
                            local r=require"ffi"
                ...
                print(encoding.UTF8:decode('v'..thisScript().version..': Не могу проверить обновление. Смиритесь или проверьте самостоятельно на '..c))
                update=false
            end
        end
    end)
    ...
    if os.clock()-f>=10 then
        print(encoding.UTF8:decode('v'..thisScript().version..': timeout, выходим из ожидания проверки обновления. Смиритесь или проверьте самостоятельно на '..c))
    end
end}]]))
